...
export default {
  ...
  methods: {
    ...
    getRank(page = 1) {
      this.page = page;
      this.$loading(1);
      let URL = "getPolkaTokenRank?page="+page
      if(this.isVoted){
        if(this.account&&this.account.address){
          URL = "getMyPolkaLike?address="+this.account.address+"&page="+(page-1);
        }else{
          this.$router.push('/');
          return false;
        }
      ...
      this.axios
        .get(
          this.domain+URL+"&sort="+this.sortByWho+"&order="+this.sortOrder+"&limit=" +this.limit
        )
        .then((res) => {
          this.$loading(0);
          if (res.data.success) {
